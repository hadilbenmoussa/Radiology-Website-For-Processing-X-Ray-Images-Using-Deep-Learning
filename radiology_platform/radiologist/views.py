...
from doctor.forms import PostForm
...
def appointmentsView(request):
    ...
    return render(
        request, "radiologist/appointments.html", {"appointments": appointments}
    )
...
def reportsView(request):
    reports = Report.objects.filter(radiologist=request.user).order_by(
        Case(
...
def generate_report(request, report_id):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            patient_info = "Name: {}\nAge: {}\nGender: {}".format(
                report.patient.get_patient_name(), 20, "Male"
            )
            ...
            report_id_info = (
                "Exam ID: {}\nDate&Time: {}\nOrdering Provider Dr: {}".format(
                    report.id, date, report.doctor
                )
            )
            ...
            addition_info = "Name: {}\nAge: {}\nGender: {}".format(
                report.exam_type, report.body_part, report.urgency
            )
            ...
            if image is not None:
                ...
                prediction = {
                    ...
                    "predictedLabel": predictedLabel[np.argmax(score)],
                    "percentage": 100 * np.max(score),
                }
                aidiagnosis = 'The classification of the scan is "{}" with "{}" % confidence'.format(
                    predictedLabel[np.argmax(score)], 100 * np.max(score)
                )
            ...
            run.add_picture(
                img_data, width=docx.shared.Inches(6), height=docx.shared.Inches(4)
            )
            print("not fine")
    ...
    return render(
        request,
        "radiologist/generate_report.html",
        {"report": report, "form": form, "prediction": prediction},
    )
...
def addblogView(request):
    if request.method == "POST":
        ...
        form = PostForm()
    return render(request, "radiologist/addblog.html", {"form": form})
